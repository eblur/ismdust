...
%% GX9p9_FeL.sl -- Examine Fe-L edge from X-ray binary GC 9+9, using
...
variable DROOT = "tgcat/obs_11072_tgid_4289/";
...
save_par("GX9p9_FeLedge.par");
write_plot("GX9p9_FeLedge.res");
...
%% What is the 1-sigma confidence interval?
...
group(meg);
write_plot("GX9p9_FeLedge.dat");
rebin_data(meg[0], 20);
rebin_data(meg[1], get_data_info(meg[0]).rebin);
xnotice( meg, ANGSMIN, ANGSMAX );

#iffalse
%% Now fit with Fe-L absorption only
...
fit_fun("FeLabs(2, powerlaw(2))");
set_par("FeLabs(2).md", 0.5, 1);
() = fit_counts;

thaw("FeLabs(2).md");
() = fit_counts;

plot_unfold(meg, pstyle; mcol=4, res=4, rcol=4);
plot_pause;

%% Write the data, model, and residuals to a file
save_par("GX9p9_FeLabs.par");
write_plot("GX9p9_FeLabs.res");

group(meg);
write_plot("GX9p9_FeLabs.dat");
rebin_data(meg[0], 20);
rebin_data(meg[1], get_data_info(meg[0]).rebin);
xnotice( meg, ANGSMIN, ANGSMAX );
...
variable mdmin2, mdmax2;
(mdmin2, mdmax2) = conf_loop( 3, 0 );

print("%%--- MD and 1-sigma confidence interval for absorption edge ---%%");
() = printf("Silicate dust mass: %.2f, (%.2f, %.2f)\n", get_par("FeLabs(2).md"), mdmin2, mdmax2);
#endif
